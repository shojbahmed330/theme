...
const ProjectsView: React.FC<ProjectsViewProps> = ({ 
  userId, currentFiles, onLoadProject, onSaveCurrent, onCreateNew 
}) => {
  ...
  return (
    <div className="flex-1 p-6 md:p-12 overflow-y-auto">
      <div className="max-w-6xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 mt-2">
              Managing {projects.length}/10 Active Stubs
            </p>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button 
              ...
              className="px-8 py-4 bg-indigo-600 rounded-2xl font-black uppercase text-[10px] text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-500 transition-all flex items-center gap-3 active:scale-95"
            >
        ...
        {/* Search Bar */}
        <div className="glass-tech p-2 rounded-2xl border-white/5 flex items-center gap-4 focus-within:border-indigo-500/30 transition-all">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <div 
                key={project.id} 
                className="glass-tech group p-6 rounded-[2.5rem] border-white/5 hover:border-indigo-500/20 transition-all flex flex-col gap-6 relative overflow-hidden"
              >
                    ...
                    {isRenaming === project.id ? (
                      <form onSubmit={(e) => handleRenameSubmit(e, project.id)} className="flex items-center gap-2">
                        <input 
                          ...
                          className="bg-black/40 border border-indigo-500/40 rounded-lg px-3 py-1 text-sm text-white outline-none w-full"
                        />
                        <button type="submit" className="text-green-400"><Save size={16}/></button>
                      </form>
                    ) : (
                      <h4 className="text-xl font-black text-white group-hover:text-indigo-400 transition-colors line-clamp-1">{project.name}</h4>
                    )}
                ...
                <div className="flex items-center gap-2 relative z-10">
                   <div className="h-1 flex-1 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-indigo-500 w-[60%] opacity-20"></div>
                   </div>
                   <span className="text-[9px] font-black uppercase text-zinc-700">{Object.keys(project.files).length} Files</span>
                </div>

                <button 
                  onClick={() => onLoadProject(project)}
                  className="w-full py-4 bg-zinc-900 border border-zinc-800 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-2 relative z-10"
                >
          ...
          <div className="glass-tech p-20 rounded-[3rem] text-center border-dashed border-zinc-800 border-2">
            <div className="w-20 h-20 bg-indigo-500/10 rounded-full flex items-center justify-center mx-auto mb-8 text-indigo-500">
               <AlertCircle size={40}/>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
           <div className="glass-tech p-10 rounded-[3rem] w-full max-w-md border-indigo-500/20 shadow-2xl relative">
              <button onClick={() => setShowModal(null)} className="absolute top-8 right-8 text-zinc-500 hover:text-white"><X size={20}/></button>
              <h3 className="text-2xl font-black text-white mb-2 uppercase tracking-tighter">{showModal === 'save' ? 'Archive' : 'New'} <span className="text-indigo-500">Project</span></h3>
              <div className="space-y-6 mt-6">
                ...
                <button disabled={isProcessing || !projectNameInput.trim()} onClick={handleAction} className="w-full py-5 bg-indigo-600 rounded-2xl font-black uppercase text-[10px] text-white shadow-lg active:scale-95 transition-all">
