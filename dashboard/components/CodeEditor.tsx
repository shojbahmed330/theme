      ...
      <aside className="w-full md:w-64 border-r border-white/5 bg-black/20 p-4 space-y-2 overflow-y-auto">
         ...
         {Object.keys(projectFiles).map(file => (
           <button key={file} onClick={() => setSelectedFile(file)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all duration-300 ${selectedFile === file ? 'bg-pink-600 text-white shadow-lg shadow-pink-600/10 scale-[1.02]' : 'text-zinc-500 hover:bg-white/5'}`}>
              <FileCode size={16} className={selectedFile === file ? 'text-white' : 'text-zinc-600'} /> {file}
           </button>
         ))}
      </aside>
      <main className="flex-1 bg-[#0c0c0e] p-4 overflow-hidden flex flex-col">
         <div className="flex items-center justify-between mb-4 px-2">
            <div className="flex items-center gap-4">
              <div className="px-4 py-1.5 bg-black/40 rounded-xl border border-white/5">
                <span className="text-xs font-mono text-pink-500 font-bold">{selectedFile}</span>
              </div>
              {onOpenConfig && (
                <button 
                  onClick={onOpenConfig}
                  className="flex items-center gap-2 text-[10px] font-black uppercase text-zinc-600 hover:text-pink-500 transition-colors"
                >
                  <Settings size={14}/> <span>App Settings</span>
                </button>
              )}
            </div>
            <button onClick={handleBuildAPK} className="px-6 py-2.5 bg-pink-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-pink-500 transition-all active:scale-95 shadow-lg shadow-pink-600/10">
         ...
         <div className="flex-1 w-full bg-black/40 border border-white/5 rounded-3xl overflow-hidden shadow-inner flex flex-col relative">
            <textarea 
              ...
              className="flex-1 w-full bg-transparent p-6 font-mono text-xs text-zinc-300 outline-none resize-none custom-scrollbar" 
              spellCheck={false}
            />
            <div className="absolute bottom-4 right-4 text-[9px] font-black uppercase text-zinc-700 tracking-widest bg-black/60 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/5">
